...
test('should render error for invalid form submission', () => {
  ...
  wrapper.find('form').simulate('submit', {
    preventDefault: () => {} // faking the prevent default function  so the test doesnt fail because of it
  });
...
test('should set description on input change', () => {
  ...
  wrapper.find('input').at(0).simulate('change', {
    target: { value } // faking the target.value
  });
...
test('should set new date on date change', () => {
  const now = moment();
  const wrapper = shallow(<ExpenseForm />);
  // prop - to use a prop on element, then we call what was returned (handler we registered) with an instance of monent
